...
## ▶ 2026-09-26 — Watchlists protect titles from Trash (issue #576): ADR-093 / DESIGN-052 / PLAN-072 drafted; interim Saves by hand
...
**Design (PR open, not merged):** ADR-093 (supersedes in part ADR-073 C-01, ADR-084 D-1/E-1, ADR-092 C-03/C-04;
...
watchlists and Save matches until S7. **Next:** an Opus design review of the docs PR (S1), then the build (S2).
Rollout order is binding: guard + Release Block verified on a live sweep, then seed the block (ledger backfill and
the three titles' legacy SAB names), then the Seerr enable (one user, then all), then re-request the three titles.
