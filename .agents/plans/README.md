...
|---|------|--------|-------------|
| 029 | Library views/grouping + Sorting & Filtering | ✅ **completed/ (v0.47.0, 2026-07-13)** | Built session 6: #243 (data/domain — released_at, per-user prefs, watch/read seam) + #245 (registry/views/facets/A–Z UX); group-card ART follow-up (owner-directed) in #249. Live-verified. |
| 032 | List-driven book automation (Kometa/lists analog) | **ESCALATED → Books Automation Saga** | Owner late-ruling 2026-07-11: lists must drive ALL THREE kinds (ebooks/audiobooks/comics) the Kometa way, and the engine leans to a **separate application** (own API; haynesnetwork UI = config+monitoring) — saga-sized design, NOT tonight. Research (#221) feeds it; no Goodreads/Hardcover accounts so LL-native Track-1 is moot; Q-06 root-caused live (GB 503 bursts + LL no-retry → design needs retry/fallback). Comics-source hunt = saga mandate. |
| 033 | Book requests + wanted-not-on-disk view | **SUBSUMED → PLAN-044** (2026-07-13) | Verdict: **adopt nothing** — Shelfarr/AudioBookRequest are their own book-*arrs (a second pipeline the governor can't see); Libreseerr fronts LL but flat-JSON roles/no approval gate; LL's native proxy-auth multi-user is a thin stopgap (request = email-the-admin). Build in-app inside the **Books Automation Saga**; the wanted-not-on-disk VIEW is requester-independent and can ship first. NO requester covers comics. |
| 039 | MAM compliance governor (cap-aware pacing) | ✅ **completed/ (v0.45.0)** | LIVE + validated: counted 13 then 14 unsatisfied as the real Matilda grab landed (threshold 15, gate open, baseline no-page); CronJob `4,19,34,49 * * * *` green. Seam = Prowlarr indexer toggle (fullSync propagates to LL). See `completed/039-*.md`. |
...
| 061 | MAM governor resume hysteresis (distinct resume floor + dead band) | **🚧 PR OPEN (do not merge — coordinator reviews docs diff)** | 2026-07-23 gate-violation fix: pause (≥ threshold 185) and resume (< floor 170) become distinct levels, dead band 170..184 holds; kills the single-threshold flap (resume at 184 → re-flood past the hard 200 cap → violation + ~26h block, 15× in 3 days). ADR-077 (supersedes ADR-054 C-05) / DESIGN-027 D-09 / R-234 / T-228/T-229. haynes-ops CronJob is suspended until the block expires + this release deploys (lift checklist in `../context/2026-07-23-mam-gate-violation-audit.md`). Took next-free 061 (041 is already `library-fix-books-and-parity`). |
| 062 | Migrator advisory lock (serialize concurrent migrate init-containers) | saga haynesnetwork-ha 01 | — | ✅ completed/ (v0.90.4) |
| 063 | Rate-limit storage in-memory → database (shared across replicas) | saga haynesnetwork-ha 05; closes O-5 | — | ✅ completed/ (v0.90.5) |
| 064 | Front-page uptime badge (Gatus SLI, ADR-079) | saga haynesnetwork-ha 04 | 062-independent | ✅ completed/ (v0.91.0) |
...
| 038 | Ticket media precision (exact episode/file linking) | ✅ **completed/ (v0.60.0, 2026-07-15)** | Locator + compose drill shipped (ADR-061/DESIGN-032); leaf-or-scope enforced; ytdlsub/books targets deferred (Q-04). |
| 035 | Ticket email notifications | ✅ **completed/ (v0.58.0, 2026-07-15)** | F-04 unblocked + shipped same day: email outbox channel (ADR-060/DESIGN-031), admin-on-create + author opt-in, live-validated on prod. |
...
| 042 | Old-WebKit login crash → Option A mitigation + compat revert | ✅ **completed/ (2026-07-13, haynes-ops-only)** | Owner ruled Option A: CSS-nesting-lowering initContainer over authentik's served assets (markers → 0), compat mode REVERTED, old-WebKit native 3/3 pass, Plex-first ordering self-healed, `%(theme)s` bg fixed. Mitigation confirmed upstream (goauthentik#19814 comment). Watch item: re-audit suspects on authentik image bumps (OPS-009). |
| 043 | **Integration Tab Saga (master)** | Saga umbrella — owner-specified 2026-07-13 | Link external accounts (Goodreads first) → pull read/watched/wanted data → coverage/sync → Missing → predictions (parked). Phases ship as numbered plans; see the plan file's phase map. |
| 044 | Goodreads requests MVP (Integrations tab + Missing) | ✅ **completed/ (ratified 2026-07-15)** | Built overnight (#253) + deployed with `sync-goodreads` CronJob. Live sync proven on the owner's real account (6 items, 5 pushed, coverage 1/6). Owner-feedback fix PR in flight (input theming, sync-on-link, comic classifier, ConfirmButton unlink). Subsumed PLAN-033. |
| 045 | Integrations hub + Goodreads library-idiom redesign | ✅ **completed/ (ratified 2026-07-15)** | Hub with per-provider sub-sections; Goodreads = stats page + library-idiom items wall with Helpdesk-style shelf chips (All + combos); all-shelves sync; **ALL shelves acquire (A1 overruled)**; every-shelf unmet items = Library WANTED tiles (books-section-gated, composed, mirror pure) **with full TV/Movies Wanted parity incl. force-search**. Fable UX build after the 044 fix PR merges; comics force-search wires to 046. |
| 046 | Kapowarr comics acquisition layer | ✅ **completed/ (ratified 2026-07-15)** | Confined `@hnet/kapowarr` client (read + import-confined write), comic-request routing (goodreads-sync stops parking when routable), comic force-search surface, stray LL want removed. Verify ComicVine key operability first (PLAN-023 owner-TODO may still be open → surfaced as unblocker, build proceeds on stubs). Never touches MAM/governor/LL-config. |
