...
| 033 | Book requests + wanted-not-on-disk view | **SURVEY DISPATCHED (owner 2026-07-11 late)** | Owner: "really looking for a Seerr type app where you request these things." Research pass evaluating existing request apps for books/audiobooks/comics (incl. LL's built-in user-accounts/request feature) on LL/Kapowarr integration + OIDC fit. Adopt-vs-build decided after, likely inside the saga. Kavita Want-To-Read confirmed library-only (no net-new requests). |
