...
| 032 | List-driven book automation (Kometa/lists analog) | **RESEARCH+DESIGN (dispatched 2026-07-11 eve)** | Owner scoped: sources = NYT + Goodreads/Listopia + Amazon + in-app/Kavita curation + a free-to-poll-API research mandate; **auto-grab + 039 governor** posture; prefer LL-native list providers if reliable. Research agent (Opus) surveying now → design doc for owner review. |
...
| 039 | MAM compliance governor (cap-aware pacing) | **BUILD (dispatched 2026-07-11 eve, Opus)** | Owner ruled governor-first (guardrail before list automation). Counts unsatisfied locally from qBittorrent, gates the MAM fallback at `limit − buffer` (20−5), fail-closed, transitions-only Pushover. Seam choice (Prowlarr indexer-pause vs LL provider toggle) decided in its ADR. |
