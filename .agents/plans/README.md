...
| 044 | Goodreads requests MVP (Integrations tab + Missing) | **SHIPPED v0.49.0 (2026-07-14); ACTIVE for acceptance tail** | Built overnight (#253) + deployed with `sync-goodreads` CronJob. Live sync proven on the owner's real account (6 items, 5 pushed, coverage 1/6). Owner-feedback fix PR in flight (input theming, sync-on-link, comic classifier, ConfirmButton unlink). Subsumed PLAN-033. |
| 045 | Integrations hub + Goodreads library-idiom redesign | **ACTIONABLE — owner-spec'd 2026-07-14 ~00:55** | Hub with per-provider sub-sections; Goodreads = stats page + library-idiom items wall with Helpdesk-style shelf chips (to-read/currently-reading/read/DNF, All + combos); all-shelves sync; to-read requests surface as Library WANTED tiles (composed, mirror stays pure). Fable UX build after the 044 fix PR merges. Assumptions A1–A3 + Q-01..Q-03 await morning confirmation. |
