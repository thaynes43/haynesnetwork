...
- **Status:** Completed (2026-07-06) — shipped v0.9.0; live-validated: real two-item
  monitor-and-search landed in Radarr (monitored flags + MoviesSearch commands verified via
  API), 600-row JSONL export byte-exact vs DB, section-permission gating (read_only/disabled)
  enforced in nav/page/controls with audited flips, search_requested events same-tx. Fileless
  import dropped on live evidence (all 4,008 backlog ids were live unmonitored rows) — reframed
  to filter + bulk monitor-and-search (ADR-022).
