...
// ADR-032 / DESIGN-004 D-16 · IA reshuffle (2026-07-09, build B) — the GENERAL tab of the tabbed
// Trash Settings hub. Admin-only (every control here is an adminProcedure). Three things:
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (
        ...
        await saveNotify.mutateAsync({ startHour: notifyStart, endHour: notifyEnd, tz: form.notifyTz });
      }
      setDraft(null);
      setSaved(true);
      void utils.trash.settings.get.invalidate();
  ...
  return (
    <section className="card batch-settings" data-testid="trash-settings">
      ...
      <div className="batch-policy" data-testid="notify-window" aria-label="Notifications">
        ...
        <p className="muted">
          Pushover pings when a Trash batch is posted and the day before it leaves. Choose the hours
          you want to be notified in — a ping raised outside the window waits until it next opens.
        </p>
        ...
        <div className="batch-settings__row">
          <div className="batch-settings__copy">
            ...
            <p className="muted">Quiet hours outside this range — pings wait until it next opens.</p>
          </div>
