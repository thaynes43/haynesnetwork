// ADR-056 (PLAN-046) — the hermetic Kapowarr stub: the REST surface the confined @hnet/kapowarr client
// drives — ComicVine volume search, the root-folder list, add-volume, volume detail (for reconcile), and the
// `auto_search` task submit — plus a call RECORDER (`/_stub/calls` + `/_stub/reset`, the stub-arr idiom) so a
// spec can assert the comic add + the force-search. Kapowarr wraps every body in `{ error, result }`. Canned
// data mirrors the live 2026-07-14 ComicVine search so the resolver picks the ORIGINAL Scott Pilgrim (Oni
// Press cv 25478) over the translated German reprint. NEVER models MAM/qBittorrent/Prowlarr (out of scope).
