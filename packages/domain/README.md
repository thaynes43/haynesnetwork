...
| `markWatched`, `dismissTitle`, `undoLastChange`              | `watch/marks.ts`           | `watch_marks` (+ the `watch_titles` write-through) | the mark rows are the audit trail; the ONLY Plex watched-state writes (`@hnet/plex/write`), never from a dismissal; undo never walks past a pending mark (a `watched` mark under ten minutes old is still working; older, it is closed and its planned keys unscrobbled, and its own finalize then leaves it alone), and never stamps a revert before the change it reverts (DESIGN-051 D-15t, D-15u) |
| `changeWatchlist` (+ the Watchlist Change half of `undoLastChange`) | `watch/watchlist.ts` (ADR-092, DESIGN-051) | `watch_marks` (`watchlist_add` / `watchlist_remove`) | the mark rows are the audit trail; the ONLY plex.tv watchlist writes (`addToWatchlist` / `removeFromWatchlist`, owner only); a change that would change nothing writes no row; one that could not be sent is still recorded (`failed`, `not sent:`) so undo closes it; with plex.tv unreadable, a title whose changes end in an unsettled call (a pending or unknown change, or a failed undo that may have moved it away from the asked state, the undo of a change of the same action, D-15z; the whole run is walked, `unsettledWatchlistRun`) gets the write, not the cache's answer (a remove so sent over a run holding an unsettled add is `after unsettled:` and never re-added by undo); undo picks a pending Watchlist Change (still working, or closed as abandoned after 60 s); an add's TMDB fallback keeps only the hits of a year the title names, takes none of another year (the pool's answer stands, or it asks), counts the pool's own exact title when TMDB's page leaves it out, and hits that all read the same are `indistinct`, never a question (D-15v, D-15w, D-15ac); an add takes the pool's title without TMDB only when it is named exactly, not another year than the one named, and known beyond a TMDB recommendation, else TMDB's exact hits decide and a pool title they cannot settle is asked about (D-15x, D-15y); undos of one account are serialized by a transaction-scoped advisory lock with a 9 s `lock_timeout`; the catalog lookup and the re-read after a failed PUT go out on the `discover` readers (one 1.5 s attempt), the live userState before the write on the short `reads` (D-15ab); the resolver's TMDB check made while the pool already has an answer goes through `tmdbOnce`, one attempt (D-15aa) (DESIGN-051 D-15, D-15k, D-15o, D-15p, D-15r, D-15s, D-15v, D-15w, D-15x, D-15y, D-15z, D-15aa, D-15ab, D-15ac) |
| `refreshWatchlistRegistry`                                   | `watchlist-registry.ts` (ADR-093, DESIGN-052 D-04) | `watchlist_registry_runs`, `watchlist_registry_accounts`, `watchlist_registry_sources`, `watchlist_registry_items`, `plex_discover_ids` | — (a derived copy of the watchlists; the run row is its record, D-21 logs never name a person) |
| `blockReleases`, `stampReleaseRecords`, `settleReleaseRecords`, `abandonReleaseRecords`, `reconcileReleaseBlock`, `checkReleaseBlockReadds` (composed by `recordAndBlockReleases`, the sweep's and Expedite's one seam), `seedReleaseBlock` | `release-block.ts`, `release-block-seed.ts` (ADR-093, DESIGN-052 D-11..D-15, D-23) | `trash_deleted_releases`; the Radarr / Sonarr release profile through `@hnet/arr/write` | the `trash_expedited` event carries `releaseRecordIds` and the claim stamps `batch_item_id` (D-25an) |
| `enrollSeerrWatchlistSync`, `setSeerrWatchlistEnroll`, `setSeerrSonarrAnimeTags` | `seerr-enroll.ts` (ADR-093, DESIGN-052 D-17) | `seerr_watchlist_enrollments` (only after the Seerr write succeeded); the `seerr_watchlist_enroll` setting through `setAppSetting` | `permission_audit` for the setting write |
| `sweepExpiredBatches` (its private `recordSweepOutcome`, `clearSweepPause`) | `trash-batches.ts` (ADR-093, DESIGN-052 D-14, D-25bf) | `trash_sweep_status` (one row: the scheduled sweep's outcome and `paused_since`) | — (the Loki `sweep_paused` / `sweep_outcome` lines) |
