...
| `changeWatchlist` (+ the Watchlist Change half of `undoLastChange`) | `watch/watchlist.ts` (ADR-092, DESIGN-051) | `watch_marks` (`watchlist_add` / `watchlist_remove`) | the mark rows are the audit trail; the ONLY plex.tv watchlist writes (`addToWatchlist` / `removeFromWatchlist`, owner only); a change that would change nothing writes no row; one that could not be sent is still recorded (`failed`, `not sent:`) so undo closes it; with plex.tv unreadable, a title whose changes end in an unsettled call (a pending or unknown change, or a failed undo; the whole run is walked, `unsettledWatchlistRun`) gets the write, not the cache's answer (a remove so sent over a run holding an unsettled add is `after unsettled:` and never re-added by undo); undo picks a pending Watchlist Change (still working, or closed as abandoned after 60 s); an add's TMDB fallback keeps only the hits of a year the title names, and hits that all read the same are `indistinct`, never a question (D-15v, D-15w); undos of one account are serialized by a transaction-scoped advisory lock with a 9 s `lock_timeout` (DESIGN-051 D-15, D-15k, D-15o, D-15p, D-15r, D-15s, D-15v, D-15w) |
