...
| `changeWatchlist` (+ the Watchlist Change half of `undoLastChange`) | `watch/watchlist.ts` (ADR-092, DESIGN-051) | `watch_marks` (`watchlist_add` / `watchlist_remove`) | the mark rows are the audit trail; the ONLY plex.tv watchlist writes (`addToWatchlist` / `removeFromWatchlist`, owner only); a change that would change nothing writes no row |
