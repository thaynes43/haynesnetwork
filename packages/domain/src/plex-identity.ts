...
//
// Home: @hnet/domain (moved from packages/auth/src/hooks/ on 2026-09-23, behavior unchanged). The
// ADR-053 Plex Account Map (T-154) is a domain seam whose "single-writer + resolution helpers" live
// here, and its reconcile runs in the sync runner, which must not pull in Better Auth. Pure — no
// imports. @hnet/auth re-exports every name unchanged, so the session path and `@hnet/auth`
