...
// ADR-086 / DESIGN-048 (PLAN-067) adds trash_save_intents — the DURABLE, REVOCABLE save intent
// keyed on the *arr identity (a Maintainerr exclusion is keyed on the ephemeral Plex ratingKey and is
// nightly-pruned when a file replacement re-keys the item, silently lapsing the Save). The @hnet/domain
// save-intent single-writer owns every write and co-writes the `trash_excluded` ledger row in the SAME
// tx (hard rule 6). Rows are REVOKED, never deleted, but the DELETE forms are guarded too so nothing
// outside the domain can quietly drop an intent. The same change closes a PRE-EXISTING omission
// (ADR-086 D-14): pending_pool_refresh claimed a @hnet/domain single writer in its schema comment but
// was never actually in this list — now guarded on all three families so the claim and the enforcement
// agree.
...
// ADR-067 / DESIGN-039 (PLAN-055 amend — D-21..D-24) adds gb_call_budget (the daily GB CALL BUDGET
// ledger — the gb_quota_state sibling that remembers per-consumer first-party GB call counts against the
// current quota-day so the estate's own consumers stay inside the shared key's low per-day cap
// unattended). recordGbCalls in gb-call-budget.ts is the sole writer (a day-rolling upsert). Derived,
// rebuildable operational state like gb_quota_state — NO audit/outbox row (routine daily weather,
// self-healing on the day roll). Upserted (insert + on-conflict update); guarded in ALL SIX families.
...
// ADR-080 (PLAN-041 Gap B — per-role media-action budgets) adds role_media_action_budgets (a role's
// hourly Fix/Force-Search budget — setRoleMediaActionBudget in media-action-budgets.ts is the sole
// writer; it upserts the row + co-writes a permission_audit 'update_media_action_budget' row in the
// SAME tx, exactly like setSectionPermission — hard rule 6). Upserted (insert + on-conflict update;
// setting 0 is still an upsert, never a delete; role deletion cascades at the DB level), so the INSERT
// / UPDATE (+ Drizzle .insert/.update) forms are guarded.
// ADR-088 / ADR-089 / DESIGN-049 D-07 (PLAN-068 — Watch Companion, migration 0077) adds five tables, all
// written ONLY by the @hnet/domain watch single-writers: watch_accounts (the tracked Plex accounts — the
// `watch` sync upserts the single `owner` row every MCP tool answers for), watch_events (the APPEND-ONLY
// Watch Event log — insert-or-ignore on (instance, tautulli_row_id), never updated), watch_titles (the
// Title State snapshot — upserted by the sync, by live revalidation and by the Watch Mark write-through;
// re-keyed in place, never deleted: a title gone from Plex keeps its event facts), watch_marks (the owner's
// explicit, reversible statements — the only path that writes Plex; the rows ARE their own audit trail,
// finalized and reverted in place) and watch_reco_signals (the recommendation input cache — each run
...
// ADR-091 / DESIGN-050 D-01 / D-03 (PLAN-069 — public OAuth connectors for the MCP surface, migration 0078) adds
// the six OAuth tables: oauth_clients, oauth_authorizations, oauth_authorization_codes, oauth_refresh_tokens,
// oauth_access_tokens and the append-only oauth_audit. @hnet/oauth is PURE (it validates and decides, it never
// touches a database); EVERY write is a @hnet/domain oauth single-writer (src/oauth/*: registerClient,
// startAuthorization, grantConsent, denyConsent, exchangeCode, rotateRefreshToken, revokeToken,
// revokeFamilyOnReuse, disconnectClient, touchLastUsed, pruneExpired), and consent grant / deny, disconnect and
// reuse revocation co-write their oauth_audit row in the SAME tx (hard rule 6). All six are guarded in ALL SIX
// families (SQL INSERT/UPDATE/DELETE + Drizzle .insert/.update/.delete): nothing outside the domain may mint a
// token, revoke one, or rewrite or drop the audit history. Inside the domain, the DELETE forms are confined
// further by `oauth-delete-paths.test.ts` — to pruneExpired (the inline pruner, D-03) and the two consent writers
// (which delete the pending request they decide, D-05 step 7).
// ADR-093 / DESIGN-052 D-05 (PLAN-072 — watchlist protection, migration 0081) adds eight tables, all written ONLY by
// @hnet/domain single-writers: the Watchlist Registry (watchlist_registry_runs / _accounts / _sources / _items and
...
const FORBIDDEN_PATTERNS: Array<{ name: string; regex: RegExp }> = [
  ...
  },
  {
    ...
    regex:
      /INSERT\s+INTO\s+(watchlist_registry_runs|watchlist_registry_accounts|watchlist_registry_sources|watchlist_registry_items|plex_discover_ids|trash_deleted_releases|seerr_watchlist_enrollments|trash_sweep_status|oauth_clients|oauth_authorizations|oauth_authorization_codes|oauth_refresh_tokens|oauth_access_tokens|oauth_audit|watch_accounts|watch_events|watch_titles|watch_marks|watch_reco_signals|trash_save_intents|pending_pool_refresh|user_role_transitions|permission_audit|roles|role_app_grants|role_section_permissions|role_trash_action_grants|role_message_action_grants|role_bulletin_view_grants|role_media_action_budgets|notifications|notification_outbox|smart_drive_state|mam_gate_state|arr_queue_cleanup_actions|library_preferences|notification_preferences|book_fix_requests|role_books_action_grants|user_account_map|user_media_watch|user_book_progress|poster_guard_applications|ai_usage_chats|authentik_users|pending_role_assignments|authentik_group_audit|books_items|media_plex_matches|plex_collections|plex_collection_members|books_collections|books_collection_members|gb_quota_state|gb_call_budget|books_format_pairs|tickets|ticket_events|ticket_replies|app_settings|trash_batches|trash_batch_items|trash_batch_saves|trash_candidates|trash_candidates_state|app_catalog|media_items|media_metadata|ledger_events|fix_requests|restore_runs|sync_runs|sync_state|plex_servers|plex_libraries|role_library_grants|role_plex_server_all_grants|plex_share_audit|user_integrations|integration_shelf_items|book_requests|role_collection_action_grants)\b/i,
  },
  {
    name: 'UPDATE guarded table (SQL)',
    regex:
      /UPDATE\s+(watchlist_registry_runs|watchlist_registry_accounts|watchlist_registry_sources|watchlist_registry_items|plex_discover_ids|trash_deleted_releases|seerr_watchlist_enrollments|trash_sweep_status|oauth_clients|oauth_authorizations|oauth_authorization_codes|oauth_refresh_tokens|oauth_access_tokens|oauth_audit|watch_accounts|watch_events|watch_titles|watch_marks|watch_reco_signals|roles|role_section_permissions|role_trash_action_grants|role_message_action_grants|role_media_action_budgets|notifications|notification_outbox|smart_drive_state|mam_gate_state|library_preferences|user_account_map|user_media_watch|user_book_progress|ai_usage_chats|authentik_users|pending_role_assignments|books_items|media_plex_matches|plex_collections|plex_collection_members|books_collections|books_collection_members|gb_quota_state|gb_call_budget|books_format_pairs|tickets|ticket_events|ticket_replies|app_settings|trash_batches|trash_batch_items|trash_batch_saves|trash_candidates|trash_candidates_state|trash_save_intents|pending_pool_refresh|app_catalog|media_items|media_metadata|ledger_events|fix_requests|restore_runs|sync_runs|sync_state|plex_servers|plex_libraries|role_library_grants|role_plex_server_all_grants|user_integrations|integration_shelf_items|book_requests)\s+SET\b/i,
  },
  {
    name: 'DELETE FROM guarded table (SQL)',
    regex:
      /DELETE\s+FROM\s+(watchlist_registry_runs|watchlist_registry_accounts|watchlist_registry_sources|watchlist_registry_items|plex_discover_ids|trash_deleted_releases|seerr_watchlist_enrollments|trash_sweep_status|oauth_clients|oauth_authorizations|oauth_authorization_codes|oauth_refresh_tokens|oauth_access_tokens|oauth_audit|watch_events|watch_marks|watch_titles|watch_reco_signals|trash_save_intents|pending_pool_refresh|role_app_grants|role_section_permissions|role_trash_action_grants|role_message_action_grants|role_bulletin_view_grants|notifications|notification_outbox|smart_drive_state|ai_usage_chats|pending_role_assignments|tickets|ticket_events|ticket_replies|app_settings|trash_batches|trash_batch_items|trash_batch_saves|trash_candidates|trash_candidates_state|roles|app_catalog|media_items|media_metadata|media_plex_matches|plex_collections|plex_collection_members|books_collections|books_collection_members|gb_quota_state|gb_call_budget|books_format_pairs|ledger_events|fix_requests|restore_runs|sync_runs|sync_state|plex_servers|plex_libraries|role_library_grants|role_plex_server_all_grants|role_collection_action_grants|plex_share_audit)\b/i,
  },
  {
    name: '.insert() into guarded/audit table (Drizzle)',
    regex:
      /\.insert\(\s*(?:[A-Za-z_$][\w$]*\.)?(watchlistRegistryRuns|watchlistRegistryAccounts|watchlistRegistrySources|watchlistRegistryItems|plexDiscoverIds|trashDeletedReleases|seerrWatchlistEnrollments|trashSweepStatus|oauthClients|oauthAuthorizations|oauthAuthorizationCodes|oauthRefreshTokens|oauthAccessTokens|oauthAudit|watchAccounts|watchEvents|watchTitles|watchMarks|watchRecoSignals|userRoleTransitions|permissionAudit|roleAppGrants|roleSectionPermissions|roleTrashActionGrants|roleMessageActionGrants|roleBulletinViewGrants|roleMediaActionBudgets|notifications|notificationOutbox|smartDriveState|mamGateState|arrQueueCleanupActions|libraryPreferences|userAccountMap|userMediaWatch|userBookProgress|posterGuardApplications|aiUsageChats|authentikUsers|pendingRoleAssignments|authentikGroupAudit|booksItems|mediaPlexMatches|plexCollections|plexCollectionMembers|booksCollections|booksCollectionMembers|gbQuotaState|gbCallBudget|booksFormatPairs|tickets|ticketEvents|ticketReplies|appSettings|trashBatches|trashBatchItems|trashBatchSaves|trashCandidates|trashCandidatesState|trashSaveIntents|pendingPoolRefresh|roles|appCatalog|mediaItems|mediaMetadata|ledgerEvents|fixRequests|restoreRuns|syncRuns|syncState|plexServers|plexLibraries|roleLibraryGrants|rolePlexServerAllGrants|plexShareAudit|userIntegrations|integrationShelfItems|bookRequests|roleCollectionActionGrants)\s*\)/,
  },
  {
    name: '.update() on guarded table (Drizzle)',
    regex:
      /\.update\(\s*(?:[A-Za-z_$][\w$]*\.)?(watchlistRegistryRuns|watchlistRegistryAccounts|watchlistRegistrySources|watchlistRegistryItems|plexDiscoverIds|trashDeletedReleases|seerrWatchlistEnrollments|trashSweepStatus|oauthClients|oauthAuthorizations|oauthAuthorizationCodes|oauthRefreshTokens|oauthAccessTokens|oauthAudit|watchAccounts|watchEvents|watchTitles|watchMarks|watchRecoSignals|users|roles|roleSectionPermissions|roleTrashActionGrants|roleMessageActionGrants|roleMediaActionBudgets|notifications|notificationOutbox|smartDriveState|mamGateState|libraryPreferences|userAccountMap|userMediaWatch|userBookProgress|aiUsageChats|authentikUsers|pendingRoleAssignments|booksItems|mediaPlexMatches|plexCollections|plexCollectionMembers|booksCollections|booksCollectionMembers|gbQuotaState|gbCallBudget|booksFormatPairs|tickets|ticketEvents|ticketReplies|appSettings|trashBatches|trashBatchItems|trashBatchSaves|trashCandidates|trashCandidatesState|trashSaveIntents|pendingPoolRefresh|appCatalog|mediaItems|mediaMetadata|ledgerEvents|fixRequests|restoreRuns|syncRuns|syncState|plexServers|plexLibraries|roleLibraryGrants|rolePlexServerAllGrants|userIntegrations|integrationShelfItems|bookRequests)\s*\)/,
  },
  {
    name: '.delete() on guarded table (Drizzle)',
    regex:
      /\.delete\(\s*(?:[A-Za-z_$][\w$]*\.)?(watchlistRegistryRuns|watchlistRegistryAccounts|watchlistRegistrySources|watchlistRegistryItems|plexDiscoverIds|trashDeletedReleases|seerrWatchlistEnrollments|trashSweepStatus|oauthClients|oauthAuthorizations|oauthAuthorizationCodes|oauthRefreshTokens|oauthAccessTokens|oauthAudit|watchEvents|watchMarks|watchTitles|watchRecoSignals|roleAppGrants|roleSectionPermissions|roleTrashActionGrants|roleMessageActionGrants|roleBulletinViewGrants|notifications|notificationOutbox|smartDriveState|aiUsageChats|pendingRoleAssignments|tickets|ticketEvents|ticketReplies|appSettings|trashBatches|trashBatchItems|trashBatchSaves|trashCandidates|trashCandidatesState|trashSaveIntents|pendingPoolRefresh|roles|appCatalog|mediaItems|mediaMetadata|mediaPlexMatches|plexCollections|plexCollectionMembers|booksCollections|booksCollectionMembers|gbQuotaState|gbCallBudget|booksFormatPairs|ledgerEvents|fixRequests|restoreRuns|syncRuns|syncState|roleLibraryGrants|rolePlexServerAllGrants|roleCollectionActionGrants|plexLibraries|plexServers)\s*\)/,
  },
];
