...
export const EMPTY_PLEX_IDENTITY: PlexIdentity = { email: null, username: null };
...
/**
 * Decode the `plex_email` / `plex_username` claims from an OIDC id_token (a JWT). The token was
 * already signature-verified by Better Auth at sign-in and now lives in our own `account` row, so
 * we decode the payload segment WITHOUT re-verifying — this is an identity HINT for share display,
 * never an authorization decision. Tolerant by design: a missing/malformed token, non-JWT string,
 * or absent claims all yield the empty identity (the caller then falls back to the override/email).
 */
 ...
 * still picks up an override email, and vice-versa. Empty fields fall through to the app email in
 * the matcher (packages/api plex router).
 */
