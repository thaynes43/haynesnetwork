...
| T-15 | Permission Bundle | **Amended by ADR-012:** the bundle is now the **Role** (T-02): its App set (T-46) plus, in Phase 3, allowed libraries / family gating as a role attribute (R-20). | `role_app_grants` (`role_libraries` prescriptive, Phase 3) | Assigning the role applies the bundle; one role per user. |
...
| T-20 | Allowed Library Set | A User's effective set of self-addable libraries. Default = all libraries except Family-Only Libraries; adjusted by admins **per role** (R-25..R-27). _(Amended by ADR-012: per-role, not per-user/per-tag — Phase 3, unbuilt.)_ | derived from the User's Role (Phase 3) | |
| T-21 | Family-Only Library | A Plex Library visible/addable only to users **in a family-designated Role**. Exactly `HNet Home Videos` and `HNet Photos` today (R-26). _(Amended by ADR-012: gated by a role attribute, not the removed `users.is_family`; see C-09.)_ | `plex_libraries.is_family_only` | Phase 3 role design owns the concrete family-gating attribute. |
