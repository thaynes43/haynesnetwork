...
| ID | Consequence |
|----|-------------|
| C-01 | **Two new tools.** `watchlist` (`watch:read`) lists the watchlist newest first with each title's kind, year and whether it is on Plex; `set_watchlist` (`watch:write`) adds or removes one title and says back the title and year it resolved. `watch_status` adds "On your watchlist." when it is. Nine tools in all. |
...
| C-03 | **Seerr coupling, by ruling.** Adding a title that is not on Plex answers that Seerr will request it. Undoing such an add removes it from the watchlist and says Seerr may already have requested it; the app never calls Seerr. This settles PRD Q-13: requests by voice go through the watchlist. |
...
| C-07 | Risk/accepted: a wrong resolution on an add of a title not on Plex downloads the wrong title within about 3 minutes, and undo cannot cancel it. Mitigated by the D-13 resolver (ambiguous asks, TMDB accepted only on an exact title), the external-id match's own title and year read back in the answer, and "Seerr will request it" said aloud. |
...
| C-09 | **Supersedes ADR-087 C-07's 3 KB cap in part: the `tools/list` cap becomes 4 KB (4,096 bytes).** Two tools and the longer descriptions come to about 3.5 KB. The cap exists for R-245 (at most 0.5 s added per voice turn); the 2,712-byte list added no measurable median latency, so the evidence is re-measured with the hass-sandbox voice bench after the deploy (PLAN-071), and R-245's latency bound, not the byte cap, stays the requirement. The 1,200-character result cap is unchanged. |
...
| C-11 | Neutral: the `watch:write` consent line gains the watchlist, so what a connector is allowed to change is said at consent. Tokens already granted keep working; the scope set is unchanged. |
