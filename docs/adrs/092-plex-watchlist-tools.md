...
| C-11 | Neutral: the `watch:read` and `watch:write` consent lines gain the watchlist, so what a connector may read and change is said at consent. Tokens already granted keep working; the scope set is unchanged. |
