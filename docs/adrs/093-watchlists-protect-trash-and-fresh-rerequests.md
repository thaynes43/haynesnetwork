...
| C-04 | **Fail closed, precisely (the Registry Gate, T-262; driver decision).** A delete needs a registry refresh that read the account roster and the owner's whole list and finished at most 30 minutes earlier (the sweep refreshes inline first), and no readable account whose last good read is older than 24 hours. A failed read never removes a title from the registry: the account's last good list is carried forward. An account that has failed continuously for 72 hours is reclassified **unreadable**, stops blocking, keeps its frozen list, and is counted. An item whose watchlist status cannot be evaluated (no `plex://` guid while some registry entry of its kind is unmapped, or Maintainerr's `ruleEvaluationFailed`) is kept as `unevaluable`. Full rules: DESIGN-052 D-07. |
| C-05 | **Documented limit: accounts whose list cannot be read do not block** (driver decision). Today that is up to 20 of 42: 3 managed users (until C-01's switch path is proven) and 17 friends whose community read is empty and who have no Seerr user, so a hidden list is indistinguishable from an empty one. Every refresh counts them; the Trash status shows the counts, never who. Any friend who signs in to Seerr once becomes readable. |
| C-06 | **Privacy.** Other people's watchlists, public or private, are guard input only: stored per account for the carry-forward rule, never shown, never exposed by an API or tool, never logged by title or by person. The Trash wall says only "On a watchlist". |
| C-07 | **Supersedes ADR-084 D-1 and E-1: the Release Block** (T-265; driver decision). Before the Maintainerr handle, the sweep and Expedite record the deleted release's identity (the **Deleted-Release Record**, T-264: grabbed release name, release group, quality, size, file name, from the *arr and the ledger) and add a derived "must not contain" term to **one app-owned release profile per Radarr and Sonarr**. The term blocks that release of that title in every post and on every indexer, so a re-request, a Kometa re-add, RSS or an upgrade picks a different release with no fetch of the deleted one. The profile is written and read back **before** the delete; if it cannot be, nothing is deleted this hour. ADR-084's unmonitor half is moot (the record is deleted); its D-2, D-3, E-3..E-6 stand, and E-6's ordering is exactly what this does. |
...
| C-10 | **The Arm/Disarm defect is fixed, and the aging invariant grows.** Every app-side rule-group save lifts the top-level-only flags (`listExclusions`, `forceSeerr`, `arrAction` and the rest) from the live group before the PUT and verifies them after. The Maintainerr safety audit (ADR-036) also requires `listExclusions` and `forceSeerr` on both rule pools, so a drift refuses the sweep instead of silently changing what a delete does. |
| C-11 | **Supersedes ADR-092 C-03 in part: everyone's watchlist requests** (ruling 3). The app turns on Seerr's watchlist sync (movies and TV) for every Seerr user through `POST /api/v1/user/{id}/settings/main`, once per user, behind an audited setting that stays off until the guard and the Release Block are verified. A user who later turns it off in Seerr is left off. ADR-092 C-03's "the app never calls Seerr" narrows to "the watch tools never call Seerr". Seerr reads each user's 20 newest titles every 3 minutes, so the first enable requests at most about 34 movies and 48 whole shows (an upper bound from every readable list), all auto-approved; TV requests ask for every season. Everything Seerr requests carries `mediarequests` and can never re-enter a Trash pool. |
...
| C-13 | **Rollout order is part of the decision** (driver decision): the guard and the Release Block ship and are verified on a live sweep; then the Release Block is seeded from the ledger's release history of past Trash deletions and from the legacy SAB names of the three watchlisted titles already deleted; then the Seerr enable (one user first, then all); then those three titles are re-requested explicitly if the enable did not request them. |
...
| C-15 | Good: a re-request never fetches the deleted release, on any path, and never costs a duplicate indexer fetch. |
| C-16 | Bad/accepted: more moving parts on the deletion path (plex.tv, community.plex.tv and Seerr must answer before a sweep deletes), and a plex.tv outage longer than the gate's bounds pauses reclaim. The pause is visible on the Trash page and in logs; the batch simply waits. |
...
| C-19 | Risk: hundreds to thousands of regex terms on every release decision; the cost is measured in PLAN-072 (DESIGN-052 Q-04) before the cap is trusted. |
