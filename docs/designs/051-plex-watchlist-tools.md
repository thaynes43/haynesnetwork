...
- **Last updated:** 2026-09-26 (D-15 records the rulings from the PR #580 code review, folded into D-02..D-12 and
  ...
  re-read after a failed PUT get one 1.5 s attempt of their own (D-03 and D-14a re-derived); D-14 the rulings made
...
D-15x and D-15y, and a seventh (findings I1..I7, verified the same way) D-15z..D-15ab. Code comments cite these IDs (and D-13 for the design review's rulings), never a ruling number.
...
- `@hnet/domain` (embedded Postgres, recording fake Plex): add on Plex, add not on Plex (Seerr line),
  ...
  undo clears the add, never re-adds it; the run walk by the asked action, D-15z), and the new actions never
...
- `@hnet/mcp` e2e: `tools/list` ≤ 4,096 bytes and the pinned exact size, nine tools, scope filtering
  ...
  discover bundle with one 1.5 s attempt (D-15ab).
