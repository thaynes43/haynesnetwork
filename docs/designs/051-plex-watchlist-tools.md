...
- **Last updated:** 2026-09-26 (D-15 records the rulings from the PR #580 code review, folded into D-02..D-12 and
  ...
  among TMDB's hits when TMDB's page leaves it out, and D-15ad that of its ninth: ChatGPT gets the new tools only
  after the owner refreshes its connector and starts a new chat (D-12 corrected, PLAN-071 S6 checks it); D-14 the
  rulings made while building; D-13 the rulings from the
...
old `watch_status` and `undo_last_change` descriptions. His grant needs no new consent, since the scope set is
unchanged (ADR-092 C-11). PLAN-071 S6 asks him to and checks it. A Claude Code or Codex session lists the tools
when it starts, so one started after the deploy has them. dev-env does need a change (corrected by D-15):
...
D-15x and D-15y, a seventh (findings I1..I7, verified the same way) D-15z..D-15ab, an eighth (findings
J1..J7, verified the same way) D-15ac, and a ninth (finding K1, verified the same way) D-15ad. Code comments cite these IDs (and D-13 for the design review's rulings), never a ruling number.
...
| D-15ad | K1 | **ChatGPT gets the new tools only after a connector refresh and a new chat.** D-12 said ChatGPT needs nothing, but ChatGPT caches a connector's tools, their descriptions and the server `instructions`, refreshes them only when the user refreshes the connector in its settings, and keeps a chat's schemas for that chat (DESIGN-050 D-12, OPS-016 §7, PLAN-069's evidence from cigar-journal's client notes). The owner's ChatGPT connector has been live since 2026-09-25 (its registration and its consent to every scope are in the web log, and it calls the read tools), so after the deploy it would keep serving the seven tools and the old `watch_status` and `undo_last_change` descriptions: "add X to my watchlist" would still fail there, the gap ADR-092 was written to close, and no plan step would have caught it. D-12 now says that after the deploy the owner refreshes the haynesnetwork connector in ChatGPT's settings and starts a new chat. No new consent is needed: the scope set is unchanged and his grant holds `watch:read` and `watch:write` (ADR-092 C-11). PLAN-071 S6 asks him to (one question on his phone), then checks the web log for a `tool_called` line with `"tool":"watchlist"` from his ChatGPT client's `oauth:` consumer and has him confirm that ChatGPT lists `set_watchlist`; the result goes in the PLAN-071 log. No `set_watchlist` call is made from ChatGPT for the check (S5 covers the write through the hop, on a title already on Plex). Claude Code and Codex list a server's tools when a session starts, so a session started after the deploy has them and needs no step. |
...
- Live (PLAN-071): the hop checks, one add/undo on a title already on Plex, the voice bench, and the owner's
  ChatGPT connector after a refresh and a new chat (`watchlist` answers, `set_watchlist` listed, D-15ad).
