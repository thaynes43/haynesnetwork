...
| D-05 | `/oauth/authorize` is a **page** (so the bad-request card renders with the app's tokens): its redirects are 307, and a rate-limited authorize shows the card with status 200. **No error redirects at all (review ruling, RFC 9700 §4.11.2):** a redirect URI registered through open DCR is not a trusted URI, so every client, redirect and parameter error renders the bad-request card, signed in or not; the only redirects to the client are the success code and `access_denied` after the user pressed Deny on the consent page. Real clients send valid requests, so losing the machine-readable error costs nothing. Order: client + redirect validation, then the session gate, then the parameters. `client_name` is required (RFC 7591 makes it optional; every known client sends it). Bound server-action arguments (the decision and the transaction id) are visible in the form; that is safe because the action re-derives the session user and the writer checks the transaction is theirs. |
| D-04 / D-10 | Bounds beyond D-04: a redirect URI ≤ 1,024 characters with no userinfo and no fragment; `client_name` is NFKC-normalised and refuses `\p{Cc}\p{Cf}\p{Zl}\p{Zp}\p{Co}\p{Cn}` (zero-width, bidi, tag and filler characters), and the consent page styles the name apart from the host line so a name cannot pose as it; `state` ≤ 1,024; `code_challenge` exactly 43 base64url characters and the verifier 43–128 unreserved characters; presented tokens ≤ 512 characters; repeated parameters refused; an authorize whose `next` would exceed 2,048 characters answers the client `invalid_request`. Client IP: `CF-Connecting-IP`, then `X-Real-IP`, then the first `X-Forwarded-For` hop; non-literals ignored; IPv6 keyed by /64 and IPv4-mapped addresses by their IPv4 (the full address is still `registered_ip`). A refresh token is issued only to a client that registered the `refresh_token` grant, even with `offline_access`. A confidential client's secret comes with `client_secret_expires_at: 0`. Metadata suffixes other than `/mcp` are 404. `/oauth/revoke` is rate-limited like `/oauth/token`. **Residual accepted:** a LAN or in-cluster client reaching `/oauth/*` through the internal ingress can spoof `CF-Connecting-IP` and mint its own rate buckets (household devices only; the tunnel path is Cloudflare-overwritten). |
| D-06 | `/oauth/revoke` answers 200 for every token outcome, but a failed client authentication is 401 `invalid_client` and a malformed request 400 `invalid_request` (RFC 7009 §2.2.1). **Spent versus revoked refresh tokens:** a *spent* (rotated) token replayed is theft — the family is revoked, `family_revoked_on_reuse` is audited (even for an already-dead family) and `refresh_reuse_detected` is logged (it pages); a *revoked* token that was **never rotated** presented again (after Disconnect, a client's own revoke, or an earlier reuse response) answers `invalid_grant` with a quiet `refresh_rejected` line and no audit row, after an unaudited re-sweep of the family — otherwise every Disconnect would page on the client's next refresh. Rotation is checked before revocation (review ruling): a token that was rotated and then revoked is still reuse, so a thief cannot silence the alert by revoking the family after rotating. A replayed authorization code also revokes every family issued from it (`family_revoked_on_reuse` with `details.reason = "code_replayed"`). When two rotations of one token race, the loser re-reads it: rotated ⇒ reuse, revoked ⇒ the quiet path. The `owner_gate_refused` event is gone; the authorize log never echoes an untrusted `client_id` (anything not shaped like one logs as `malformed`), and `audience_mismatch` logs the supplied resource as origin plus path, capped, with token-like runs masked. |
...
| D-08 | The admin view is one list of every user's connections (the admin's own included) with a user line; an admin may Disconnect any row and the audit `details` record `actor_user_id`. The row shows the host the user actually consented to (recorded as `redirect_host` in the `consent_granted` audit details), not the client's first registered URI — a malicious client could register `chatgpt.com` first and take codes at its second URI. |
| D-03 (pruning) | `pruneExpired` never deletes a client that was ever used (`last_used_at` set): ChatGPT caches its `client_id` per connector, and pruning it after a Disconnect or an idle spell would break the reconnect with `invalid_client`. Only never-used clients older than 30 days with no rows go. `oauth_audit.user_id` is `ON DELETE SET NULL` like the repo's other audit tables. |
| D-07 (challenges) | The 401 carries `error="invalid_token"` when a token was presented and rejected (none when no token was sent); the 403 carries `scope="<the required scope>"` beside `error="insufficient_scope"`. |
| D-02 / app | Every response carries `X-Frame-Options: DENY` and `Content-Security-Policy: frame-ancestors 'none'` (RFC 9700 §4.16: a sibling `*.haynesnetwork.com` app with an XSS could otherwise frame a signed-in consent page and click Approve). No page in the app is meant to be framed. |
