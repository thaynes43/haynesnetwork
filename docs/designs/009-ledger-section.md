...
with Movie/TV/Music sub-tabs via `?tab=` (WAI-ARIA tablist, default Movies — mirrors `/library`).
...
- **Monitor & search:** the bar button opens a **`Modal`** (explanatory confirm — ADR-014 /
  ...
  **`ok`/`outcome`**, the search badge off **`searched`** (never off error text — D-05).
  ...
  without titles and never enter `preview`). A **Recent runs** card lists `ledgerAdmin.runs`
  and reopens any report; runs are synchronous, so fetch-on-load suffices (no polling).
