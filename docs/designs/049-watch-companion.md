...
| D-02 | The body cap answers **413**; an unparseable body 400. `serverInfo.version` is the root `package.json` version (release-please bumps it; `APP_VERSION` overrides). The per-request server is closed after the JSON response is built. |
