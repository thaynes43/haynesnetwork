...
| `@hnet/mcp` (**new**) | request handler, consumer auth, tool registry, budgets, logging | depends on `@modelcontextprotocol/sdk` **pinned to 1.30.0** (zod `^3.25 \|\| ^4` peer, so zod 4.4.3 is fine), `@hnet/watch`, `@hnet/domain`; its tests use embedded Postgres and the SDK `Client` |
...
5. Writes (`/:/scrobble?identifier=com.plexapp.plugins.library&key=<ratingKey>`): whole show ⇒ the
   ...
   unwatched before. *Season-key rule (live incident 2026-09-23, D-26):* specials (season 0) never take
   part and the show key is never written — a whole show is written season by season (seasons ≥ 1), and a
   season key only when every leaf under it is unwatched, else each unwatched episode key, because Plex's
   container scrobble also bumps `viewCount` and re-stamps `lastViewedAt` on every already-watched leaf.
...
| D-03 | *Ruling (PR #563 review):* `markWatched`, `dismissTitle`, `undoLastChange` and `revalidateTitles` act only for the current `owner` row (one primary-key SELECT); any other account (none stored yet, or a demoted `household` row) throws `WatchNotReadyError`, whose message is "Watch history isn't ready yet.", before any Plex call or write. |
...
| D-08 | *Ruling:* `name:<kind>:<normalized>\|<year>`. The writer re-keys a row only to a STRONGER key no other row holds and no earlier input of the same batch is inserting, and an input matching two stored rows (two rows created before a key linked them) updates the strongest-keyed one. Guid-less show events (Q-06) key as `name:show:<show title>\|<year>` for each year they carry (an episode's Tautulli `year` is its own air year, not the show's) and, when that matches nothing, join the one Plex or stored show with the same normalized title. |
...
| D-14 / D-15 | *Ruling (live incident 2026-09-23; supersedes the D-14 row's specials sentence and the D-15 row's show-key collapse):* a whole-show mark of The Expanse (all 62 episodes watched) scrobbled the show key: it flipped 7 specials, and Plex also bumped `viewCount` and re-stamped `lastViewedAt` on every already-watched episode under the key. So season 0 never takes part in a mark (before-state, `flipped`, writes; a season below 1 asks for one), the show key is never scrobbled or unscrobbled, and a season key is scrobbled only when every leaf under it is unwatched, else each unwatched episode key. Nothing unwatched in seasons ≥ 1 ⇒ no write (`written`, `flipped = []`, "was already watched …, all N episodes"). Undo collapses only to a season ≥ 1 key whose every leaf is in `flipped`. |
| D-14 / D-15 / D-09 | *Ruling (live incident 2026-09-23; supersedes the D-14 row's `plex_counts` sentence):* after a mark or an undo that sent a write, a show's `plex_counts` entries for those servers are dropped, so the next sync (and revalidation) re-reads its leaves there even when Plex's counters end where they started; a movie's written servers take the applied state instead, so the absent-movie check catches a Plex that disagrees. Dates are not nulled: with no write re-stamping a watched leaf, the undo's live read already carries the pre-mark dates, and a re-read recomputes them from Plex leaves and events alone (no stored date, writer maximum or mark takes part). The Expanse's "finished today" is Plex's own data (HaynesOps' 62 re-stamped episodes), which no Plex API can backdate. |

### D-27 — Rulings made while building the MCP surface and the local stack (PLAN-068 S7–S8)

| Section | Ruling |
|---|---|
| D-05 | **`tools/list` is served from hand-written JSON Schemas** through the low-level `ListToolsRequestSchema` handler: the SDK-generated list (a `$schema` URL and `execution: {taskSupport}` on every tool) measured **3,475 bytes**, over the 3,072-byte budget; the served list is **2,712 bytes** (same D-05 names, descriptions and annotations). The hand-written schemas carry the parameters, types, enums and integer bounds; string lengths are checked by zod only; a test pins each to its zod schema (`z.toJSONSchema`). Bounds chosen where D-05 gave none: `season` 1–100, `episode` 1–9999, `offset` 0–100, `genre` ≤ 40 characters, `title` 1–200. |
| D-05 / D-06 | The strict zod schema validates inside the tool runner, not in the SDK (which is handed an accept-anything schema), so an invalid call is logged like any other (`"code":"invalid_args"`) and answered `isError` with "Invalid arguments for <tool>: <path>: <zod message>." — paths and messages only, never an argument value. A tool outside the consumer's scopes is not registered on its connection: `tools/list` omits it and a call to it answers the SDK's "not found" (the runner's `"code":"scope"` refusal only backs that up). |
| D-02 | The body cap answers **413**; an unparseable body 400. `serverInfo.version` is the root `package.json` version (release-please bumps it; `APP_VERSION` overrides). The per-request server is closed after the JSON response is built. *Review of S7 (2026-09-23):* a JSON-RPC **batch** (an array body) is refused with 400 `-32600` — batching left MCP in 2025-06-18, none of the three consumers batch, and a batch carrying a `notifications/cancelled` for its own call made the SDK drop the response and the request hang. Every request races a **9 s deadline** (`MCP_DEADLINE_MS`, under Home Assistant's 10 s per call): on expiry the server is closed, a `tools/call` answers the D-06 error text with `isError: true` (a normal result, which the SDK client and HA hand to the model as any tool error), any other request gets 504, and the call's one `tool_called` line carries `code: "deadline"`. The abandoned work is not cancelled — a Plex write that lands late still finalizes its mark, which is why marks are idempotent and a retry replays (D-14 step 7) — and nothing it does afterwards is logged. |
| D-03 | The consumer registry is `MCP_CONSUMERS` (name, token env, scopes) — a second consumer is one entry; every configured token is compared (no early exit). 401/503 bodies are JSON-RPC-shaped errors. |
| D-06 | `slow_call` names the slowest measured phase: the flows report `resolve` and `plex_write`, the MCP layer `revalidate`. `revalidate_timeout` is logged when D-11 ran out of budget. |
| D-10 / D-21 | `unfinished` with `kids: true` lists children's titles alongside the rest (D-10: "not children's unless kids"); `recommend` with `kids: true` returns only children's picks (D-18). `recent_history` groups a window's events per title and leaves out a title with a live `not_mine` mark (someone else's viewing — the D-15 spirit extended to the history answer). |
| D-11 | Revalidation runs on a Plex bundle built with a 300 ms per-attempt timeout and no retry delay (the 400 ms overall deadline bounds the GET retries); marks on one with 800 ms per attempt and a 100 ms retry delay. Missing Plex configuration degrades (no revalidation; a mark is recorded with Plex `failed`), missing TMDB configuration skips the resolver's fallback. |
| D-17 | The library pre-filter's genre overlap is a substring `LIKE ANY` over every source spelling of the canonical genre (so "Sci-Fi & Fantasy" matches `sci-fi`); children's titles are pre-filtered by a Kids/Children genre (`kids: false`) or a Kids/Children/Family/Animation genre (`kids: true`); ordering is `COALESCE(imdb, tmdb, rt/10)` then title. "New on Plex" is the earliest `media_plex_matches.first_seen_at`. Watchlist and seed candidates take on-Plex, genres and ratings from their ledger match (by TMDB / TVDB / IMDb id). |
| PLAN-068 S8 | `pnpm dev:local` (not the e2e harness) runs a one-row demo seed (Stub Severance's ledger row and Plex match) and the real `watch` sync once at start; the stack env carries `HNET_MCP_HOP_TOKEN=local-dev-mcp-hop-token`. The stub history adds a fully watched show (caught up — the stub ledger has no Sonarr "ended" for it), a Taster and an on-Plex watchlist show. |
